...
func blackWhiteGameCellIterator(cell BlackWhiteGameCell, adjacentCells *[]*BlackWhiteGameCell) *BlackWhiteGameCell {
	newCell := cell
...
func initSetBlackWhiteGameCells(g ggol.Game[BlackWhiteGameCell]) {
	...
	for x := 0; x < size.Width; x++ {
		for y := 0; y < size.Height; y++ {
			...
			g.SetCell(&c, BlackWhiteGameCell{Alive: (x+y)%2 == 0})
		}
	}
}
